...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') return res.status(200).json({ ok: true, hint: "POST { texts: ['one','two'] }" });
  if (req.method !== 'POST') return res.status(405).json({ error: 'Use POST' });

  try {
    const body = parseBody(req);
    const list = Array.isArray(body.texts) ? body.texts : [];
    if (!list.length) return res.status(400).json({ error: "Provide JSON: { texts: ['Great quarter', 'Worried about margins'] }" });
